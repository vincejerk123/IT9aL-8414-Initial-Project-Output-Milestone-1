...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Edit Your Information</h1>
                <p class="text-gray-500">Position: {{ $application->job->title }}</p>
            </div>
        </div>

        <form action="{{ route('applications.updateStep1', $application->id) }}" method="POST">
            @csrf
            
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Full Name *</label>
                    <input type="text" name="full_name" required value="{{ old('full_name', $application->full_name) }}"
                ...
                <div>
                    ...
                    <input type="email" name="email" required value="{{ old('email', $application->email) }}"
                           class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition">
                </div>
                
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Phone Number *</label>
                    <input type="tel" name="phone" required value="{{ old('phone', $application->phone) }}"
                           class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition">
                </div>
            </div>
